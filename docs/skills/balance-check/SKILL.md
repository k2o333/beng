...
Read relevant data/config files for the identified domain.
...
Read the GDD / design doc for the system to understand intended design targets,
...
After presenting the report, ask the user what to do next:
...
- Guide the user to update the relevant data file or formula
- After each fix, offer to re-run the relevant balance checks to verify no new outliers were introduced
- If the fix changes a tuning knob defined in a design document, remind the user to
  propagate the change through affected docs before committing.

If [B]:
- Write the report to `design/balance/balance-check-[system]-[date].md` (create the
  directory if needed). Use current date for `[date]` in YYYY-MM-DD format.
...
> 本地化备注（人脉圈项目）：本 skill 的流程已适配为 `docs/drafts/alpha4/02-calibration-and-balance-report.md`
> 的 `npm run balance` 静态报告 + sim.js 模拟方案；数据源为 `src/js/data/balance.js`、设计文档为
> `docs/drafts/alpha3/09-full-gdd.md`。原版 frontmatter 中的 `agent: economy-designer`、
> `model: sonnet` 等宿主专用字段已按本仓库需要精简。
